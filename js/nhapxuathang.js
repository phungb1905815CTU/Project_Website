...
formInputProduct.addEventListener('submit',async (event) => {
    ...
    const nhaphangSony = ` https://api.thingspeak.com/update?api_key=9PGDZYDEO90HVCWO&field1=${inputProductSony.value}`;
...
formOutputProduct.addEventListener('submit',async (event) => {
    event.preventDefault();
    const xuathangSony = `https://api.thingspeak.com/update?api_key=05R5KXIAOZYWMC6Q&field1=${outputProductSony.value}`;
